...
assert fields['name']=='in.nightwise.demo' and fields['versionName']=='0.11.0-live-preview' and fields['versionCode']=='26'
...
target=root/'output/apk/nightwise-0.11.0-team-debug.apk'
...
record={'file':target.name,'sizeBytes':target.stat().st_size,'sha256':digest,'packageId':fields['name'],'versionName':fields['versionName'],'versionCode':26,'signatureVerified':True,'webAssetsMatch':True,'serverKeyAbsent':True,'backgroundLocationPermission':False,'verificationScope':'APK packaging only; native and provider acceptance are recorded separately in talks/records'}
(target.parent/'nightwise-0.11.0-team-verification.json').write_text(json.dumps(record,indent=2),encoding='utf-8')
