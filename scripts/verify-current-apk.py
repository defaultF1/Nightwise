...
assert fields['name']=='in.nightwise.demo' and fields['versionName']=='0.7.7-preview' and fields['versionCode']=='17'
...
target=root/'output/apk/nightwise-0.7.7-debug.apk'
...
record={'file':target.name,'sizeBytes':target.stat().st_size,'sha256':digest,'packageId':fields['name'],'versionName':fields['versionName'],'versionCode':17,'signatureVerified':True,'webAssetsMatch':True,'serverKeyAbsent':True,'backgroundLocationPermission':False,'verificationScope':'APK packaging only; native and provider acceptance are recorded separately in talks/records'}
(target.parent/'nightwise-0.7.7-verification.json').write_text(json.dumps(record,indent=2),encoding='utf-8')
