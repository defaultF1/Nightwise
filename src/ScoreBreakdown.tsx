...
export function ScoreBreakdown({comparison,routeId,sample}:{comparison:Comparison|null;routeId:string;sample:boolean}){
  ...
  return <section className="score-breakdown" aria-label="Score breakdown">
    ...
    <dl className="evidence-rows">{(Object.keys(WEIGHTS) as Component[]).map(key=>{
      ...
      return included?<div key={key}><dt>{labels[key]}{key==='mainRoad'&&<small>About {Math.round(100*values[key]!)}% of the route follows mapped main roads.</small>}{key==='cameras'&&comparison.cameraEvidence?.[routeId]&&<small>{comparison.cameraEvidence[routeId].count} eligible mapped camera sites within 60 m. Data: {comparison.cameraEvidence[routeId].dataTimestamp.slice(0,10)}.</small>}</dt><dd>{`${(weights[key]*values[key]!).toFixed(1)} / ${weights[key]} pts`}</dd></div>:null;
    })}</dl>
