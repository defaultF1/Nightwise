...
export function LocalDirectory({directory,rows,pinCount,show,onShow}:{directory:Directory;rows:(DirectoryPlace&{routeDistanceMeters:number})[];pinCount:number;show:boolean;onShow:(show:boolean)=>void}){
 ...
 return <section className="local-directory" aria-label="Local place directory">
  ...
  <p className="settings-helper">{pinCount} additional pins close to this route. Browse {rows.length} saved listings within 200 m below, including nearby streets. Saved {directory.savedAt.slice(0,10)}. These pins do not affect live activity scores; opening times and entrances may have changed.</p>
