...
import { camerasNearRoute } from './domain/camera-layer';
...
export function App({ initialTheme, initialAccessCode = '' }: { initialTheme: Theme; initialAccessCode?: string }) {
  ...
  const comparison = providerComparison
    ? compareActivity(rawRoutes,analyses,dataMode==='sample'?fixtureRoadEvidence(rawRoutes,scenario):Object.fromEntries(rawRoutes.map(r=>[r.id,{...liveResult?.roadAnalyses?.[r.id],...(r.turns!==undefined?{maneuversPerKm:r.turns/(r.distanceMeters/1000)}:{})}])),{allowLive:true,allowEstimates:usesGeoapify&&dataMode==='live',maxExtraMinutes:extraMinutes,mode:journey.mode}) : providerComparison;
  const routes=useMemo(()=>labelRoutes(rawRoutes,comparison??undefined,Object.fromEntries(rawRoutes.map(r=>[r.id,usesGeoapify?camerasNearRoute(r.path).length:0]))),[rawRoutes,comparison]);
