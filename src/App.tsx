...
export function App({ initialTheme }: { initialTheme: Theme }) {
  ...
  const comparison = providerComparison
    ? compareActivity(routes,analyses,dataMode==='sample'?fixtureRoadEvidence(routes,scenario):Object.fromEntries(routes.map(r=>[r.id,{...liveResult?.roadAnalyses?.[r.id],...(r.turns!==undefined?{maneuversPerKm:r.turns/(r.distanceMeters/1000)}:{})}])),{allowLive:true,maxExtraMinutes:extraMinutes}) : providerComparison;
          ...
          {!routes.length ? <section className="state-panel"><Illustration name="route-unavailable"/><h2>No routes in this preview</h2><p>Try another starting point or change the tutorial scenario.</p><button className="secondary-button" onClick={cancel}>Edit journey</button></section> : <>
            ...
            <div className="route-list" role="radiogroup" aria-label="Select a route">
              {[...routes].sort((a,b)=>Number(b.id===comparison?.recommendedId)-Number(a.id===comparison?.recommendedId)||(comparison?.scores[b.id]??-1)-(comparison?.scores[a.id]??-1)||a.durationSeconds-b.durationSeconds||a.id.localeCompare(b.id)).map(route => <div key={route.id} className={`route-card ${selectedRoute?.id === route.id ? 'selected' : ''}`}>
                <label className="route-select"><input type="radio" name="route" aria-label={route.label} checked={selectedRoute?.id === route.id} onChange={() => setSelected(route.id)} /><span className="route-content"><span className="route-card-top"><span>{route.label}{comparison?.rankedIds?.includes(route.id)?` · Comparison rank ${comparison.rankedIds.indexOf(route.id)+1}`:''}</span><span className="selection-check" aria-hidden="true">{selectedRoute?.id === route.id ? <Check size={14} /> : null}</span></span>{comparison?.recommendedId===route.id&&<span className="activity-label recommendation-label">More listed activity</span>}{comparison?.scores[route.id]!==undefined&&<span className="activity-score">{comparison?.commonComponents.includes('openDensity')?'Night Activity Score':'Road information score'} <b>{Math.round(comparison.scores[route.id])}/100</b><small>Based on {comparison.commonComponents.length} of 6 signals</small></span>}<span className="route-duration">{minutes(route)} <small>min</small></span><span className="route-distance">{(route.distanceMeters / 1000).toFixed(1)} km {minutes(route) > fastest ? <span> · +{minutes(route) - fastest} min</span> : <span> · quickest option</span>}</span>{!!route.evidence.openPlaces&&<span className="route-summary">{`${route.evidence.openPlaces} places listed as open around your passing time`}</span>}</span></label>
