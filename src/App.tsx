...
import { currentLocation, openMaps } from './native';
...
import { ActivityStrip, formatMeters } from './ActivityStrip';
...
export function App({ initialTheme }: { initialTheme: Theme }) {
  ...
  async function compare(nextScenario: TutorialScenario = scenario) {
    ...
    try {
      ...
      const result = await routeProvider.getRoutes({origin:'AEOS',destinationId:'aeos',mode:'DRIVE',scenario:nextScenario},current.signal);
  ...
  const handoffUrl = mapsHandoff(dataMode==='sample'?DEFAULT_JOURNEY:journey, selectedRoute);
  ...
  async function checkConnection() {
    ...
    catch { setConnection('Backend not reachable. Start the NightWise API on this computer; use USB forwarding on Android.'); }
  }
  ...
  return <><div className="app-shell" inert={splash}>
    <header className="app-header">
      ...
      <div className="header-actions"><span className="city-label"><MapPin size={14} /> Bengaluru</span><button className="icon-button" aria-label="Open settings" onClick={() => setModal('settings')}><Settings2 size={21} /></button></div>
    </header>
    ...
    <main className={view === 'plan' ? 'plan-main' : 'journey-main'} aria-busy={view === 'loading'}>
      {view === 'plan' ? <>
        ...
        <section className="plan-card" aria-labelledby="journey-title">
          <div className="section-top"><h2 id="journey-title">Where are you heading?</h2><span className="small-label">BENGALURU</span></div>
          <div className="place-fields"><div className="field-marker"><span /><i /><MapPin size={19} /></div><div className="field-controls">
            <button className="place-field" onClick={() => setModal('origin')}><span><small>FROM</small><strong>{origin}</strong></span><ChevronDown size={18} /></button>
            <button className="place-field destination" onClick={() => setModal('destination')}><span><small>TO</small><strong>{destination}</strong><span className="secondary-address">Bengaluru · selected destination</span></span><Building2 size={20} /></button>
          </div></div>
          <button className="text-button location-action" onClick={() => setModal('location')}><LocateFixed size={16} /> Use my location</button>
          <div className="journey-tools"><button className="secondary-button" onClick={()=>{locationId.current++;setLocating(false);cancel();setJourney(j=>({...j,origin:j.destination,destination:j.origin}));setDataMode('live');}}>Swap origin and destination</button><button className="text-button" onClick={()=>{cancel();setJourney(DEFAULT_JOURNEY);}}>AEOS → Manyata preset</button></div>
          ...
          <div className="trip-settings"><span><CarFront size={19} /> Driving</span><span><Clock3 size={17} /> {dataMode==='sample'?'8:30 pm · sample':'Leave now'}</span></div>
          <button className="primary-button" onClick={() => dataMode==='live'?setModal('confirm'):void compare()}>Compare night routes <ArrowRight size={20} /></button>
          <div className="tutorial-note"><Info size={16} /><p>{dataMode==='sample'?<>Sample routes and activity. No Google requests are made in this mode.</>:<>Live Google routes use the pilot allowance. Activity may remain unassessed if scans are off or incomplete.</>}</p></div>
        </section>
        {dataMode==='live'&&<div className="plan-map"><button className="secondary-button" onClick={()=>setShowMap(v=>!v)}>{showMap?'Hide map':'Show Bengaluru map'}</button>{showMap&&!modal&&<LiveMap routes={[]} onSelect={()=>{}} journey={journey} theme={theme} blocked={splash}/>}</div>}
        ...
        <div className="journey-heading"><div><h1 id="page-heading" tabIndex={-1}>{view === 'loading' ? 'Getting your preview ready' : view === 'error' ? 'Let’s try that again' : 'Your route options'}</h1><p>{origin} <ArrowRight size={14} /> {destination}</p></div><span className="sample-badge">{dataMode==='sample'?'Sample data':'Live request'}</span></div>
        {view === 'loading' ? <section className="state-panel" role="status"><div className="loading-ring" /><h2>{stage==='routes'?(dataMode==='sample'?'Loading sample routes':'Requesting live routes and evidence'):'Calculating activity evidence'}</h2><p>{dataMode==='live'?'Checking the journey within the pilot allowance. This can take up to 90 seconds.':stage==='routes'?'Preparing the selected tutorial journey.':'Sampling the paths and interpreting the supplied test observations.'}</p><div className="analysis-stages"><span className={stage==='activity'?'done':''}>{stage==='activity'?<Check size={15}/>:<span className="stage-dot"/>} Route options</span><span>{stage==='activity'?<span className="stage-dot"/>:<span className="stage-pending"/>} Activity and comparison</span></div><button className="secondary-button" onClick={cancel}>Cancel</button></section>
        : view === 'error' ? <section className="state-panel"><Illustration name="connection-retry"/><h2>We couldn’t load routes</h2><p>{errorMessage} Your journey is saved.</p><button className="primary-button" onClick={() => { setScenario('normal'); void compare('normal'); }}>{dataMode==='sample'?'Retry sample journey':'Retry live journey'} <ArrowRight size={18} /></button><button className="text-button" onClick={cancel}>Back to journey</button></section>
        : <div className="results-layout">{dataMode==='live'?<LiveMap routes={routes} selectedId={selectedRoute?.id} onSelect={setSelected} journey={journey} theme={theme} blocked={!!modal||splash} analysis={selectedAnalysis}/>:<RouteDiagram routes={routes} selectedId={selectedRoute?.id} onSelect={setSelected} origin={origin} destination={destination}/>}<div className="results-panel">
          {!routes.length ? <section className="state-panel"><Illustration name="route-unavailable"/><h2>No routes in this preview</h2><p>Try another starting point or change the tutorial scenario.</p><button className="secondary-button" onClick={cancel}>Edit journey</button></section> : <>
            <div className="freshness"><span>{dataMode==='sample'?'Illustrative check: 9 Sep · 8:30 pm IST':`Checked ${liveResult?new Date(liveResult.checkedAt).toLocaleString('en-IN',{timeZone:'Asia/Kolkata',day:'numeric',month:'short',hour:'numeric',minute:'2-digit'}):'just now'} IST`}{stale?' · Over five minutes old':''}</span><button className="text-button" onClick={()=>void compare()}>{dataMode==='sample'?'Refresh sample':'Refresh live results'}</button></div>
            <p className="preference-note">Considering up to {extraMinutes} extra minutes. All returned routes remain selectable.</p>
            <EvidenceConfidence analysis={selectedAnalysis} road={liveResult?.roadAnalyses?.[selectedRoute?.id]}/>
            <details className="help-details"><summary>Help points on selected route</summary><HelpPoints analysis={selectedAnalysis}/></details>
            {liveResult&&<details className="live-notices"><summary>Live evidence and pilot usage</summary>{liveResult.notices.map(n=><p key={n}>{n}</p>)}<p>{liveResult.usage.routeCalls}/{liveResult.usage.routeLimit} route calls · {liveResult.usage.nearbyCalls}/{liveResult.usage.nearbyLimit} nearby requests used in this pilot.</p>{liveResult.attributions.map((a,i)=><p key={i}>{a.uri?<a href={a.uri} target="_blank" rel="noreferrer">{a.name}</a>:a.name}</p>)}</details>}
            {comparison&&<div className="evidence-notice" data-outcome={comparison.outcome}><Info size={18}/><p>{comparison.message}</p></div>}
            ...
            <div className="route-list" role="radiogroup" aria-label="Select a route">
              {[...routes].sort((a,b)=>Number(b.id===comparison?.recommendedId)-Number(a.id===comparison?.recommendedId)||(comparison?.scores[b.id]??-1)-(comparison?.scores[a.id]??-1)||a.durationSeconds-b.durationSeconds||a.id.localeCompare(b.id)).map(route => <div key={route.id} className={`route-card ${selectedRoute?.id === route.id ? 'selected' : ''}`}>
                <label className="route-select"><input type="radio" name="route" aria-label={route.label} checked={selectedRoute?.id === route.id} onChange={() => setSelected(route.id)} /><span className="route-content"><span className="route-card-top"><span>{route.label}{comparison?.rankedIds?.includes(route.id)?` · Activity rank ${comparison.rankedIds.indexOf(route.id)+1}`:''}</span><span className="selection-check" aria-hidden="true">{selectedRoute?.id === route.id ? <Check size={14} /> : null}</span></span>{comparison?.recommendedId===route.id&&<span className="activity-label recommendation-label">{dataMode==='sample'?'More listed activity in this sample':'More listed activity · experimental'}</span>}{comparison?.scores[route.id]!==undefined&&<span className="activity-score">Night Activity Score <b>{Math.round(comparison.scores[route.id])}/100</b><small>{comparison.commonComponents.length}/6 signals · {comparison.commonComponents.length<6?'Partial · ':''}{dataMode==='sample'?'Sample · uncalibrated':'Experimental · uncalibrated'}</small></span>}{dataMode==='live'&&comparison?.scores[route.id]===undefined&&<span className="score-unavailable">Night Activity Score <b>Unavailable</b><small>A validated score is not available for this live route. Review the evidence below.</small></span>}<span className="route-duration">{minutes(route)} <small>min</small></span><span className="route-distance">{(route.distanceMeters / 1000).toFixed(1)} km {minutes(route) > fastest ? <span> · +{minutes(route) - fastest} min</span> : <span> · quickest option</span>}</span><span className="route-summary">{route.evidence.openPlaces===null?'Activity evidence is unavailable':`${route.evidence.openPlaces} observed open listings · ${Math.round(route.evidence.coverage*100)}% scan coverage`}</span></span></label>
                {analyses.find(a=>a.routeId===route.id)&&<ActivityStrip analysis={analyses.find(a=>a.routeId===route.id)!}/>}
    ...
    <footer className="app-footer"><span>NightWise · Bengaluru, India</span><button onClick={() => setModal('about')}>How it works</button></footer>
    ...
    {modal === 'evidence' && selectedRoute && selectedAnalysis && <Sheet title="A closer look at this route" onClose={() => setModal(null)}><span className="sample-badge">{dataMode==='sample'?'Calculated from sample observations':'Live Google listings · estimates'}</span><p className="evidence-lead">{selectedRoute.label} · {minutes(selectedRoute)} minutes<br/><span>Here’s what the evidence can tell us.</span></p>{!selectedAnalysis.coreComparable&&<div className="unknown-explanation"><Illustration name="evidence-unknown" compact/><p>Some evidence is missing. Unassessed areas are unknown, not low activity.</p></div>}<ActivityStrip analysis={selectedAnalysis}/><dl className="evidence-rows"><div><dt>Observed places listed as open</dt><dd>{selectedAnalysis.openPlaces ?? 'Unavailable'}</dd></div><div><dt>Observed places listed as closed</dt><dd>{selectedAnalysis.closedPlaces ?? 'Unavailable'}</dd></div><div><dt>Places with unknown hours</dt><dd>{selectedAnalysis.unknownHours ?? 'Unavailable'}</dd></div><div><dt>Open staffed-place categories</dt><dd>{selectedAnalysis.staffedPlaceProxy ?? 'Unavailable'}</dd></div><div><dt>Longest stretch without an open help listing</dt><dd>{formatMeters(selectedAnalysis.longestHelpGapMeters)}</dd></div><div><dt>Help listings scheduled open around arrival</dt><dd>{selectedAnalysis.potentialHelpPoints ?? 'Unavailable'}</dd></div><div><dt>Transport locations listed as open</dt><dd>{selectedAnalysis.openTransportPoints ?? 'Unavailable'}</dd></div><div><dt>Listed places closing soon</dt><dd>{selectedAnalysis.closingSoon ?? 'Unavailable'}</dd></div><div><dt>Longest low-activity stretch</dt><dd>{formatMeters(selectedAnalysis.longestLowActivityMeters)}</dd></div><div><dt>Total low-activity distance</dt><dd>{formatMeters(selectedAnalysis.totalLowActivityMeters)}</dd></div><div><dt>Observed turns</dt><dd>{selectedRoute.turns??'Not assessed'}</dd></div><div><dt>Scan coverage by distance</dt><dd>{Math.round(selectedAnalysis.scanCoverage*100)}%</dd></div><div><dt>Activity evidence coverage</dt><dd>{Math.round(selectedAnalysis.activityCoverage*100)}%</dd></div><div><dt>Known opening hours</dt><dd>{selectedAnalysis.openPlaces===null?'Unavailable':`${Math.round(selectedAnalysis.hoursCoverage*100)}%`}</dd></div><div><dt>{dataMode==='sample'?'Illustrative check time':'Evidence checked'}</dt><dd>{dataMode==='sample'?'9 Sep · 8:30 pm IST':new Date(selectedAnalysis.checkedAt).toLocaleTimeString('en-IN',{timeZone:'Asia/Kolkata',hour:'numeric',minute:'2-digit'})+' IST'}</dd></div></dl><ScoreBreakdown comparison={comparison} routeId={selectedRoute.id} sample={dataMode==='sample'}/><RoadEvidence analysis={liveResult?.roadAnalyses?.[selectedRoute.id]}/><div className="evidence-footnotes"><p>Counts are distinct observed listings, not a census. Staffed-place categories are a proxy for businesses normally attended by staff; actual staff presence is not measured. Low activity means fewer than two confirmed-open listings within a 150 m sample radius. This initial threshold needs local calibration. The gap estimate comes from samples about 200 m apart and is not a surveyed street measurement.</p><p>Lighting and crime are not measured. Road classification is shown separately when available. Potential help categories do not verify access, staff or assistance.</p>{selectedAnalysis.limitations.map(text=><p key={text}>{text}</p>)}<p>Opening hours are evaluated at an estimated passing time using available schedules. Traffic, access and actual opening can differ. Scores are experimental and uncalibrated. Live scores remain withheld until provider-use review and local validation are resolved.</p></div><button className="secondary-button" onClick={() => setModal(null)}>Back to routes</button></Sheet>}
    {modal === 'handoff' && <Sheet title="Continue in Google Maps" onClose={() => setModal(null)}><span className="place-icon large"><Navigation size={26} /></span><p className="evidence-lead">{origin}<br /><span>to {destination}</span></p><p className="selected-handoff">Selected: {selectedRoute?.label} · {selectedRoute ? minutes(selectedRoute) : 0} min {dataMode==='sample'?'in this sample':''}</p><p>{dataMode==='sample'?'Google Maps will calculate a current route between the supplied pins. Illustrative paths are not transferred.':'We send up to three points along your selected route to help preserve its corridor. Maps may show these as stops or recalculate the path.'}</p>{stale&&<p role="status">The comparison is over five minutes old. Check current conditions in Maps.</p>}<p className="settings-helper">Check the starting gate, entrance, stops and route preview before navigating. Opening Google Maps sends the journey locations to Google.</p><a className="primary-button" href={handoffUrl} target="_blank" rel="noreferrer" onClick={e=>{if(Capacitor.isNativePlatform()){e.preventDefault();void openMaps(handoffUrl).catch(()=>setLocationMessage('Could not open Maps. Try again or use a browser.'));}}}>Open Google Maps <ExternalLink size={18} /></a>{locationMessage&&<p role="status">{locationMessage}</p>}<button className="text-button centered" onClick={() => setModal(null)}>Keep comparing</button></Sheet>}
    {modal === 'location' && <Sheet title="Use your current location" onClose={()=>{locationId.current++;setLocating(false);setModal(null);}}><p className="sheet-intro">Use a one-time location reading as the starting pin. This switches to live mode. Nothing is sent to the backend until you compare.</p><p>You can keep using the supplied AEOS pin without granting location permission.</p>{locationMessage&&<p role="status">{locationMessage}</p>}<button className="primary-button" disabled={locating} onClick={()=>void locate()}>{locating?'Locating…':'Get current location'}</button><button className="text-button centered" onClick={()=>{locationId.current++;setLocating(false);setModal('origin');}}>Choose a pin instead</button></Sheet>}
    {modal === 'about' && <Sheet title="About NightWise" onClose={() => setModal(null)}><div className="about-brand"><MoonMark size={40} /><strong>Compare night routes.</strong></div><p>NightWise helps you compare travel time with available activity information along a journey. It does not measure personal safety, lighting or crime risk.</p><Illustration name="activity-explained-v2"/><h3>About this tutorial</h3><p>Tutorial mode uses illustrative routes, durations and observations. Live mode requests Google routes and, when enabled, nearby listings through our backend. Both AEOS and Manyata pins come from your supplied links. Missing information stays unknown.</p><h3>How comparisons work</h3><p>We sample along each path, count each observed place once and keep unknown sections separate. Extra time is shown alongside evidence. A substantial detour or incomplete data prevents an automatic activity recommendation.</p><h3>Your privacy</h3><p>No account is needed. This build remembers your appearance, extra-time preference and saved-place labels on this device. Google favourites store a place ID and are resolved again when selected. Pins you enter yourself stay locally until removed. It does not track your location continuously, send analytics or run background scans. A one-time location reading is optional. Live comparisons send journey coordinates to our backend and Google. Place searches send the text you explicitly submit to our backend and Google when enabled. Results stay in memory for the session; only request counts are saved by the backend. OpenStreetMap road data is stored locally; your route is not written to that extract. A Google Maps handoff shares the chosen journey with Google.</p><button className="secondary-button" onClick={() => setModal(null)}>Back to NightWise</button></Sheet>}
