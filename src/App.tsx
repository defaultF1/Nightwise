import { useCameraExpiry } from './use-camera-expiry';
...
export function App({ initialTheme }: { initialTheme: Theme }) {
  ...
  // Public access: no startup gate, stored code or code-entry field.
  const accessCode = '';
  ...
  const cameraNow = useCameraExpiry(analyses);
  const comparison = providerComparison
    ? compareActivity(rawRoutes,analyses,journey.mode==='WALK'?{}:dataMode==='sample'?fixtureRoadEvidence(rawRoutes,scenario):Object.fromEntries(rawRoutes.map(r=>[r.id,{...liveResult?.roadAnalyses?.[r.id],...(r.turns!==undefined?{maneuversPerKm:r.turns/(r.distanceMeters/1000)}:{})}])),{allowLive:true,maxExtraMinutes:extraMinutes,now:cameraNow}) : providerComparison;
  const routes=useMemo(()=>rawRoutes.map(r=>({...r,label:comparison?.recommendedId===r.id?(comparison.rankedIds?.[0]===r.id?'Highest scoring route':'Higher scoring route'):comparison?.fastestId===r.id?'Fastest route':r.label})),[rawRoutes,comparison?.recommendedId,comparison?.fastestId,comparison?.rankedIds?.[0]]);
  ...
  const handoffUrl = mapsHandoff(journey,selectedRoute,comparison?.fastestId??undefined);
  ...
  return <><div className="app-shell" inert={splash}>
    ...
    <main className={view === 'plan' ? 'plan-main' : 'journey-main'} aria-busy={view === 'loading'}>
      {view === 'plan' ? <>
        ...
        <p className="quiet-note">Safety Score is an estimate, not a safety guarantee. <button onClick={() => setModal('about')}>About NightWise</button></p>
      </> : <>
        ...
        : <><JourneySummary routes={routes} comparison={comparison} analyses={analyses} onSelect={setSelected}/><div className="results-layout">{dataMode==='live'?<LiveMap routes={routes} selectedId={selectedRoute?.id} onSelect={setSelected} journey={journey} theme={theme} blocked={!!modal||splash} analysis={selectedAnalysis} cameraNow={cameraNow} activityStatus={liveResult?.activityStatus}/>:<RouteDiagram routes={routes} selectedId={selectedRoute?.id} onSelect={setSelected} origin={origin} destination={destination}/>}<div className="results-panel">
          {!routes.length ? <section className="state-panel"><Illustration name="route-unavailable"/><h2>No routes in this preview</h2><p>Try another starting point or change the tutorial scenario.</p><button className="secondary-button" onClick={cancel}>Edit journey</button></section> : <>
            ...
            <div className="route-list" role="radiogroup" aria-label="Select a route">
              {[...routes].sort((a,b)=>Number(b.id===comparison?.recommendedId)-Number(a.id===comparison?.recommendedId)||(comparison?.scores[b.id]??-1)-(comparison?.scores[a.id]??-1)||a.durationSeconds-b.durationSeconds||a.id.localeCompare(b.id)).map(route => <div key={route.id} className={`route-card ${selectedRoute?.id === route.id ? 'selected' : ''}`}>
                <label className="route-select"><input type="radio" name="route" aria-label={route.label} checked={selectedRoute?.id === route.id} onChange={() => setSelected(route.id)} /><span className="route-content"><span className="route-card-top"><span>{route.label}{comparison?.rankedIds?.includes(route.id)?` · Comparison rank ${comparison.rankedIds.indexOf(route.id)+1}`:''}</span><span className="selection-check" aria-hidden="true">{selectedRoute?.id === route.id ? <Check size={14} /> : null}</span></span>{comparison?.recommendedId===route.id&&<span className="activity-label recommendation-label">Higher Safety Score</span>}{comparison?.scores[route.id]!==undefined&&<ScoreRing score={comparison.scores[route.id]} label="Safety Score" signals={comparison.commonComponents.length}/>}<span className="route-summary">{(()=>{const cameras=summarizeCameras(route,analyses.find(a=>a.routeId===route.id)?.cameras,cameraNow);return cameras?`${cameras.count} mapped cameras · ${cameras.perKm.toFixed(1)} per km`:'Camera evidence unavailable';})()}</span><span className="route-duration">{minutes(route)} <small>min</small></span><span className="route-distance">{(route.distanceMeters / 1000).toFixed(1)} km {minutes(route) > fastest ? <span> · +{minutes(route) - fastest} min</span> : <span> · quickest option</span>}</span>{!!route.evidence.openPlaces&&<span className="route-summary">{`${route.evidence.openPlaces} places listed as open around your passing time`}</span>}</span></label>
            ...
            {!!selectedRoute?.listedPlaces?.length&&<details className="help-details"><summary>Mappls listings near this route ({selectedRoute.listedPlaces.length})</summary><section className="help-points"><p>Listed within 150 m of this route. Distances run along the route from your start. Exact positions and opening status are not supplied by the current Mappls plan.</p><ul>{selectedRoute.listedPlaces.map(p=><li key={p.id}><strong>{p.name}</strong><span>{({shop:'Shop',medical:'Pharmacy / clinic',hospital:'Hospital',fuel:'Petrol / CNG'} as const)[p.kind]}{p.alongRouteMeters!==undefined?` · about ${formatMeters(p.alongRouteMeters)} from your start`:''}</span><span>{p.address}</span></li>)}</ul></section></details>}
            ...
            <p className="route-disclaimer"><Info size={15} /> Safety Score is an estimate, not a safety guarantee.</p>
    ...
    {modal === 'about' && <Sheet title="About NightWise" onClose={() => setModal(null)}><div className="about-brand"><MoonMark size={40} /><strong>Compare night routes.</strong></div><p>NightWise helps you compare travel time with available activity information along a journey. It does not measure personal safety, lighting or crime risk.</p><Illustration name="activity-explained-v2"/><h3>About this tutorial</h3><p>Tutorial mode walks through AEOS ↔ Manyata Tech Park using a built-in street map. Live mode requests Mappls routes and, when enabled, nearby listings through our backend. Both AEOS and Manyata pins come from your supplied links. Missing information stays unknown.</p><h3>How comparisons work</h3><p>We sample along each path, count each observed place once and estimate opening hours for the moment you pass. The Safety Score combines up to seven signals: open places, main roads, help points, activity continuity, fewer turns, transport and mapped traffic cameras. Cameras receive 15% when all seven signals are available; the remaining signals keep their previous relative importance. Missing signals are excluded equally for every route. Camera locations do not confirm that cameras are working or monitored. The score is a product estimate, not a validated measure of crime risk. A substantial detour prevents an automatic recommendation.</p><h3>Your privacy</h3><p>No account is needed. This build remembers your appearance, extra-time preference and saved-place labels on this device. Saved places store a Mappls place code and are resolved again when selected. Pins you enter yourself stay locally until removed. It does not track your location continuously, send analytics or run background scans. A one-time location reading is optional. Live comparisons send journey coordinates to our backend and Mappls. In live search, suggestions are requested after you pause typing. Search text and the opposite journey endpoint are sent to our backend and Mappls to show place matches. Results stay in memory for the session; only request counts are saved by the backend. OpenStreetMap road data is stored locally; your route is not written to that extract. A Mappls handoff shares the chosen journey with Mappls.</p><button className="secondary-button" onClick={() => setModal(null)}>Back to NightWise</button></Sheet>}
