...
import { mapsHandoff, handoffWaypoints, type NavigationApp } from './domain/handoff';
...
export function App({ initialTheme, initialAccessCode = '' }: { initialTheme: Theme; initialAccessCode?: string }) {
  ...
  const [navigationApp,setNavigationApp]=useState<NavigationApp>(usesGeoapify?'mappls':'google');
  ...
  const routes=useMemo(()=>rawRoutes.map(r=>({...r,label:comparison?.recommendedId===r.id?(comparison.rankedIds?.[0]===r.id?'Most active route':'More active route'):comparison?.fastestId===r.id?'Fastest route':r.label})),[rawRoutes,comparison?.recommendedId,comparison?.fastestId,comparison?.rankedIds?.[0]]);
