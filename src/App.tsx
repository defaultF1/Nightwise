...
export function App({ initialTheme }: { initialTheme: Theme }) {
  ...
  async function compare(nextScenario: TutorialScenario = scenario) {
    ...
    try {
      if (dataMode === 'live') {
        const result = await liveComparison(journey, current.signal);
