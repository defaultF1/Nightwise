...
import { mapsHandoff, handoffWaypoints } from './domain/handoff';
import { labelRoutes } from './domain/route-labels';
import { camerasNearRoute } from './domain/camera-layer';
import { currentLocation, chooseNavigation, openLocationSettings, openAppSettings, setNativeBackground } from './native';
...
export function App({ initialTheme, initialAccessCode = '' }: { initialTheme: Theme; initialAccessCode?: string }) {
  ...
  const comparison = providerComparison
    ? compareActivity(rawRoutes,analyses,journey.mode==='WALK'?{}:dataMode==='sample'?fixtureRoadEvidence(rawRoutes,scenario):Object.fromEntries(rawRoutes.map(r=>[r.id,{...liveResult?.roadAnalyses?.[r.id],...(r.turns!==undefined?{maneuversPerKm:r.turns/(r.distanceMeters/1000)}:{})}])),{allowLive:true,allowEstimates:usesGeoapify&&dataMode==='live',maxExtraMinutes:extraMinutes}) : providerComparison;
  ...
  const handoffUrl = mapsHandoff(journey,selectedRoute,comparison?.fastestId??undefined,'mappls');
  const hasHandoffPoints=selectedRoute?.id!==comparison?.fastestId&&handoffWaypoints(journey,selectedRoute).length>0;
  ...
  return <><div className="app-shell" inert={splash}>
    ...
    <main className={view === 'plan' ? 'plan-main' : 'journey-main'} aria-busy={view === 'loading'}>
      {view === 'plan' ? <>
        ...
        <section className="plan-card" aria-labelledby="journey-title">
          ...
          <div className="trip-settings">{dataMode==='live'?<><label><CarFront size={19}/><span>Travel by<select aria-label="Travel mode" value={journey.mode} onChange={e=>{setJourney(j=>({...j,mode:e.target.value as LiveJourney['mode']}));cancel();}}><option value="DRIVE">Car</option><option value="TWO_WHEELER">Bike (motorbike)</option><option value="WALK">Walk</option></select></span></label><label><Clock3 size={17}/><span>Departure<select aria-label="Departure time" value={journey.departureTime?'later':'0'} onChange={e=>{const n=Number(e.target.value);setJourney(j=>({...j,departureTime:n?new Date(Date.now()+n*60000).toISOString():undefined}));cancel();}}><option value="0">Leave now</option>{journey.departureTime&&<option value="later">{new Date(journey.departureTime).toLocaleTimeString('en-IN',{hour:'numeric',minute:'2-digit',timeZone:'Asia/Kolkata'})} IST</option>}{[15,30,60,120,180,240,300].map(n=><option key={n} value={n}>In {n<60?`${n} min`:`${n/60} hour${n===60?'':'s'}`}</option>)}</select></span></label></>:<><span><CarFront size={19}/> Driving</span><span><Clock3 size={17}/>8:30 pm</span></>}</div>
    ...
    {modal === 'handoff' && <Sheet title="Start navigation" onClose={() => setModal(null)}><span className="place-icon large"><Navigation size={26} /></span><p className="evidence-lead">{origin}<br /><span>to {destination}</span></p>{Capacitor.isNativePlatform()?<p className="settings-helper">Your phone will ask which installed maps app to use. That app plans its own route from your current location to the destination; compare it with your selected route before travelling.</p>:<p className="settings-helper">Opens the route preview in a new tab. Choose {journey.mode==='WALK'?'walking':journey.mode==='TWO_WHEELER'?'your motorbike mode':'driving'} there before starting.</p>}<p className="selected-handoff">Selected: {selectedRoute?.label} · {selectedRoute ? minutes(selectedRoute) : 0} min {dataMode==='sample'?'':''}</p>{!Capacitor.isNativePlatform()&&<p>{hasHandoffPoints?'Route points help your navigation app follow the selected roads. They may appear as stops. Keep them in place and check the preview; roads between them may change.':'Navigate from your starting point to your destination, without added stops.'}</p>}{journey.departureTime&&<p className="settings-helper">Planned departure: {new Date(journey.departureTime).toLocaleString('en-IN',{timeZone:'Asia/Kolkata'})} IST. Set this time again in the navigation app.</p>}{stale&&<p role="status">The comparison is over five minutes old. Check current conditions in your navigation app.</p>}<p className="settings-helper">Check the starting gate, entrance, stops and route preview before navigating. Starting navigation shares your journey locations with the maps app you choose.</p><a className="primary-button" href={handoffUrl} target="_blank" rel="noreferrer" onClick={e=>{if(Capacitor.isNativePlatform()){e.preventDefault();void chooseNavigation({latitude:journey.destination.latitude,longitude:journey.destination.longitude,name:journey.destination.name},handoffUrl).catch(()=>setLocationMessage('Could not open a maps app. Try again or use a browser.'));}}}>Start navigation <ExternalLink size={18} /></a>{locationMessage&&<p role="status">{locationMessage}</p>}{dataMode==='live'&&<div className="route-feedback">{routeFeedback?<p role="status">{routeFeedback}</p>:<><span>Was this suggestion helpful?</span><button onClick={()=>void rateRoute('up')} aria-label="Helpful">👍</button><button onClick={()=>void rateRoute('down')} aria-label="Not helpful">👎</button></>}</div>}<button className="text-button centered" onClick={() => setModal(null)}>Keep comparing</button></Sheet>}
