...
type Modal = 'settings' | 'origin' | 'destination' | 'evidence' | 'handoff' | 'about' | 'location' | null;
...
export function App({ initialTheme }: { initialTheme: Theme }) {
  ...
  async function checkConnection() {
    ...
    try { const s=await serviceStatus(); setConnection(s.paused?'Backend connected. Google requests are paused while billing is pending.':s.ready?`Backend ready. Activity scans ${s.activityEnabled?'enabled':'off'}. Live scoring ${s.scoringEnabled?'experimental':'pending validation'}.`:'Backend connected. Add the restricted server key to enable live routes.'); }
    ...
    <main className={view === 'plan' ? 'plan-main' : 'journey-main'} aria-busy={view === 'loading'}>
      {view === 'plan' ? <>
        ...
        <section className="plan-card" aria-labelledby="journey-title">
          <div className="section-top"><h2 id="journey-title">Where are you heading?</h2><span className="small-label">NORTH BENGALURU</span></div>
          ...
          <button className="primary-button" onClick={() => void compare()}>Compare night routes <ArrowRight size={20} /></button>
        ...
        {dataMode==='live'&&<div className="plan-map"><button className="secondary-button" onClick={()=>setShowMap(v=>!v)}>{showMap?'Hide map':'Show Bengaluru map'}</button>{showMap&&<LiveMap routes={[]} onSelect={()=>{}} journey={journey} theme={theme} blocked={!!modal||splash}/>}</div>}
        ...
        : <div className="results-layout">{dataMode==='live'?<LiveMap routes={routes} selectedId={selectedRoute?.id} onSelect={setSelected} journey={journey} theme={theme} blocked={!!modal||splash} analysis={selectedAnalysis}/>:<RouteDiagram routes={routes} selectedId={selectedRoute?.id} onSelect={setSelected} origin={origin} destination={destination}/>}<div className="results-panel">
          {!routes.length ? <section className="state-panel"><Illustration name="route-unavailable"/><h2>No routes in this preview</h2><p>Try another starting point or change the tutorial scenario.</p><button className="secondary-button" onClick={cancel}>Edit journey</button></section> : <>
            {stale&&<div className="evidence-notice" role="status"><p>These results are over five minutes old. Refresh before travel.</p><button className="text-button" onClick={()=>void compare()}>Refresh live results</button></div>}
