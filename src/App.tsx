...
import TravelModePicker, { closeTravelModePicker } from './TravelModePicker';
...
export function App({ initialTheme, initialAccessCode = '' }: { initialTheme: Theme; initialAccessCode?: string }) {
  ...
  useEffect(() => {
    ...
    const listener = NativeApp.addListener('backButton', () => {
      ...
      if (closeTravelModePicker()) return;
  ...
  const handoffUrl = mapsHandoff(journey,selectedRoute,comparison?.fastestId??undefined,navigationApp);
  const hasHandoffPoints=navigationWaypoints(journey,selectedRoute,comparison?.fastestId??undefined,navigationApp).length>0;
  ...
  return <><div className="app-shell" inert={splash}>
    ...
    <main className={view === 'plan' ? 'plan-main' : 'journey-main'} aria-busy={view === 'loading'}>
      {view === 'plan' ? <>
        ...
        <section className="plan-card" aria-labelledby="journey-title">
          ...
          <div className="trip-settings">{dataMode==='live'?<><TravelModePicker value={journey.mode} onChange={mode=>{setJourney(j=>({...j,mode}));cancel();}}/><label><Clock3 size={17}/><span>Departure<select aria-label="Departure time" value={journey.departureTime?'later':'0'} onChange={e=>{const n=Number(e.target.value);setJourney(j=>({...j,departureTime:n?new Date(Date.now()+n*60000).toISOString():undefined}));cancel();}}><option value="0">Leave now</option>{journey.departureTime&&<option value="later">{new Date(journey.departureTime).toLocaleTimeString('en-IN',{hour:'numeric',minute:'2-digit',timeZone:'Asia/Kolkata'})} IST</option>}{[15,30,60,120,180,240,300].map(n=><option key={n} value={n}>In {n<60?`${n} min`:`${n/60} hour${n===60?'':'s'}`}</option>)}</select></span></label></>:<><span><CarFront size={19}/> Driving</span><span><Clock3 size={17}/>8:30 pm</span></>}</div>
    ...
    {modal === 'handoff' && <Sheet title="Start navigation" onClose={() => setModal(null)}>
      ...
      <fieldset className="navigation-apps"><legend>Navigate with</legend>{([['google','Google Maps'],['mappls','Mappls']] as const).map(([id,name])=><label key={id} className={navigationApp===id?'selected':''}>
        <input type="radio" name="navigation-app" value={id} checked={navigationApp===id} onChange={()=>{setNavigationApp(id);setNavigationError('');}}/>{name}
      </label>)}</fieldset>
      ...
      <a className="primary-button" href={handoffUrl} target="_blank" rel="noreferrer" onClick={e=>{setNavigationError('');if(Capacitor.isNativePlatform()){e.preventDefault();void openMaps(handoffUrl).catch(()=>setNavigationError('Could not open directions. Try the other navigation app.'));}}}>Start navigation <ExternalLink size={18} /></a>
