      ...
      {view === 'plan' ? <>
        ...
        <section className="plan-card" aria-labelledby="journey-title">
          ...
          <div className="place-fields"><div className="field-marker"><span /><i /><MapPin size={19} /></div><div className="field-controls">
            <button className="place-field" disabled={dataMode==='sample'} onClick={() => setModal('origin')}><span><small>FROM</small><strong>{origin}</strong></span><ChevronDown size={18} /></button>
            <button className="place-field destination" disabled={dataMode==='sample'} onClick={() => setModal('destination')}><span><small>TO</small><strong>{destination}</strong><span className="secondary-address">Bengaluru · selected destination</span></span><Building2 size={20} /></button>
          </div></div>
