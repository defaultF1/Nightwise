...
import { formatGap } from './ActivityStrip';
...
export function HelpPoints({analysis}:{analysis?:ActivityAnalysis}){
  if(!analysis)return <section className="help-points"><h3>Help points along this route</h3><p>Unavailable in this response. Nearby pharmacy, hospital, petrol-pump, police and hotel opening hours have not been assessed.</p><p>Longest gap without a listed open help point: <strong>Unknown</strong></p></section>;
  const points=helpPoints(analysis);
  return <section className="help-points"><h3>Help points along this route</h3><p>Listed as open around the estimated passing time. Opening schedules do not confirm assistance or access.</p><p>Longest gap without a listed open help point: <strong>{formatGap(analysis.helpGapBounds,analysis.longestHelpGapMeters)}</strong></p>
