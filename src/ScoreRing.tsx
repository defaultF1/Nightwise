...
export function ScoreRing({score,label,signals}:{score:number;label:string;signals:number}){
 ...
 return <span className="score-display" aria-label={`${label}: ${value} out of 100. Based on ${signals} of 6 signals.`}>
  ...
  </span><span className="score-caption">{label}<small>Based on {signals} of 6 signals</small></span>
 </span>;
}
