...
export function ScoreRing({score,label,signals}:{score:number;label:string;signals:number}){
 const value=Math.round(Math.max(0,Math.min(100,score)));
 return <span className="score-display" aria-label={`${label}: ${value} out of 100. Based on ${signals} of 7 signals.`}>
  ...
  </span><span className="score-caption">{label}<small>Based on {signals} of 7 signals</small></span>
 </span>;
}
