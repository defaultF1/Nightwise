...
export const PILOT_RADIUS_METERS=20000;
...
export function inBengaluru(p: Coordinate): boolean {
  return !!p && Number.isFinite(p.latitude) && Number.isFinite(p.longitude) && p.latitude >= 12.75 && p.latitude <= 13.35 && p.longitude >= 77.30 && p.longitude <= 77.90;
}
