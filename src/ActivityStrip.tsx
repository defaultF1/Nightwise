...
export function formatMeters(value:number|null):string{
  if(value===null)return 'Not assessed';if(value===0)return 'None observed';
...
export function ActivityStrip({analysis}:{analysis:ActivityAnalysis}){
  ...
  return <div className="activity-strip">
    ...
    <div className="segment-band" role="img" aria-label={`${Math.round(known)} percent of the ${analysis.source==='sample'?'sample ':''}route has usable activity evidence. ${Math.round(100-known)} percent is unknown.`}>
      {analysis.segments.map((segment,i)=><span key={i} className={`segment ${segment.state}`} style={{width:`${(segment.toMeters-segment.fromMeters)/analysis.distanceMeters*100}%`}} title={`${Math.round(segment.fromMeters)}–${Math.round(segment.toMeters)} m: ${segment.state==='active'?'Open listing observed':segment.state==='low'?'No confirmed open listing':'Unknown evidence'}`}/>)}
    </div>
    <div className="segment-legend"><span><i className="active"/> Open listing</span><span><i className="low"/> Low observed</span><span><i className="unknown"/> Unknown</span></div>
  </div>;
}
