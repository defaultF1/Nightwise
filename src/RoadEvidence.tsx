...
export function RoadEvidence({analysis}:{analysis?:RoadAnalysis}){
  ...
  return <section className="road-evidence" aria-label="Road type evidence"><h3>Road type estimate</h3><dl className="evidence-rows">
    ...
    {analysis.internalTurns!==undefined&&<div><dt>Estimated turns onto internal roads</dt><dd>{analysis.internalTurns}</dd></div>}
    {analysis.unknownTurns!==undefined&&analysis.unknownTurns>0&&<div><dt>Turns with unknown road type</dt><dd>{analysis.unknownTurns}</dd></div>}
