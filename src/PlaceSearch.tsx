...
export function PlaceSearch({onChoose,anchor,direction,accessCode}:{onChoose:(point:JourneyPoint)=>void;anchor:JourneyPoint;direction:SearchDirection;accessCode:string}){
  ...
  useEffect(()=>()=>cancel(),[]);
  useEffect(()=>{if(query.trim().length>=3)timer.current=setTimeout(()=>void search(),700);return()=>clearTimeout(timer.current);},[query,anchor.latitude,anchor.longitude,direction]);
