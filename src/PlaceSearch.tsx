...
export function PlaceSearch({onChoose,accessCode,anchor,direction,mode='DRIVE',departureTime}:{onChoose:(point:JourneyPoint)=>void;accessCode:string;anchor:JourneyPoint;direction:SearchDirection;mode?:import('./domain/types').TravelMode;departureTime?:string}){
  ...
  return <><form className="place-search" onSubmit={e=>{e.preventDefault();if(query.trim().length>=3&&!busy&&!choosing)void search();}}>
    ...
    <button className="secondary-button" type="submit" disabled={busy||choosing||query.trim().length<3}>{busy?'Searching…':`Search ${city}`}</button><p className="settings-helper">Suggestions appear after you pause typing. Travel estimates use your selected mode, departure and journey endpoints.</p></form>
    ...
    {suggestions.length>0&&<section id="place-results" aria-label="Google place results"><p className="search-label">Google Maps</p><p className="search-trip-context">{mode==='WALK'?'Walking':mode==='TWO_WHEELER'?'Motorbike':'Driving'} · {tripLabel}</p>{suggestions.map(p=>{
