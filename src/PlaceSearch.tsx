...
export function PlaceSearch({onChoose,accessCode,anchor,direction,mode='DRIVE',departureTime}:{onChoose:(point:JourneyPoint)=>void;accessCode:string;anchor:JourneyPoint;direction:SearchDirection;mode?:import('./domain/types').TravelMode;departureTime?:string}){
  ...
  useEffect(()=>()=>cancel(),[]);
  useEffect(()=>{if(query.trim().length>=3)timer.current=setTimeout(()=>void search(),700);return()=>clearTimeout(timer.current);},[query,accessCode,anchor.latitude,anchor.longitude,direction,mode,departureTime]);
  ...
  async function search(){
    ...
    try{
      ...
      setPreviewState('Checking travel times…');
      previewTimer.current=setTimeout(()=>{void (async()=>{
        try{const result=await previewPlaces(found.map(p=>p.id),token.current,coordinate,direction,c.signal,accessCode,mode,departureTime);if(id!==sequence.current||c.signal.aborted)return;setEstimates(result.estimates);setCheckedAt(result.checkedAt);setPreviewState('');}
        catch{if(id===sequence.current&&!c.signal.aborted)setPreviewState('Travel estimates unavailable. You can still choose a place.');}
      })();},900);
  ...
  return <><form className="place-search" onSubmit={e=>{e.preventDefault();if(query.trim().length>=3&&!busy&&!choosing)void search();}}>
    ...
    <button className="secondary-button" type="submit" disabled={busy||choosing||query.trim().length<3}>{busy?'Searching…':`Search ${city}`}</button><p className="settings-helper">{usesGeoapify?'Suggestions appear after you pause typing. Travel estimates use your mode and endpoints, with approximated traffic.':'Suggestions appear after you pause typing. Travel estimates use your selected mode, departure and journey endpoints.'}</p></form>
    ...
    {suggestions.length>0&&<section id="place-results" aria-label={usesGeoapify?'Place results':'Google place results'}><p className="search-label">{placesAttribution}</p><p className="search-trip-context">{mode==='WALK'?'Walking':mode==='TWO_WHEELER'?'Motorbike':'Driving'} · {tripLabel}</p>{suggestions.map(p=>{
      ...
      return <button className="origin-option search-result" key={p.id} disabled={choosing} onClick={()=>void choose(p)}><span><strong>{p.title}</strong><small>{p.address}</small>
        {estimate?.available?<span className="search-travel"><b>{distanceText(estimate.distanceMeters!)} · {timeText(estimate.durationSeconds!)}</b><small>{mode==='WALK'?'Estimated walk':mode==='TWO_WHEELER'?'Estimated ride':'Estimated drive'}</small></span>:<span className="search-travel"><small>{Number.isFinite(p.straightDistanceMeters)?`${distanceText(p.straightDistanceMeters!)} straight-line distance · `:''}{previewState==='Checking travel times…'?'Checking travel time…':'Travel time unavailable'}</small></span>}
      </span></button>;
