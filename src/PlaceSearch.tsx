...
export function PlaceSearch({onChoose,accessCode}:{onChoose:(point:JourneyPoint)=>void;accessCode:string}){
  ...
  return <><form className="place-search" onSubmit={e=>{e.preventDefault();if(query.trim().length>=3&&!busy)void search();}}>
    <label className="search-label" htmlFor="place-query">Search places</label><input className="origin-search" id="place-query" value={query} onChange={e=>editQuery(e.target.value)} placeholder="Try AEOS or Manyata Tech Park" maxLength={100} autoComplete="off"/>
