...
export function ShopHours({analysis}:{analysis?:ActivityAnalysis}){
  return <details className="help-details shop-hours"><summary>Shop opening times and closed days</summary><p>{analysis?.source==='sample'?'Times are in Bengaluru time (IST). Expand a place to see its opening time, closing time and weekly schedule.':'Times are in Bengaluru time (IST). Current schedules include special-day changes reported by Google; regular schedules describe a typical week. A special date does not automatically mean a holiday or closure.'}</p>
