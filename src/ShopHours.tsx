...
export function ShopHours({analysis}:{analysis?:ActivityAnalysis}){
  return <details className="help-details shop-hours"><summary>Shop opening times and closed days</summary><p>Times are in Bengaluru time (IST). Current schedules include special-day changes reported by Google; regular schedules describe a typical week. A special date does not automatically mean a holiday or closure.</p>
  {!analysis?.places.length?<p>No shop schedules were returned for this comparison.</p>:<div className="shop-hours-list">{analysis.places.map(p=>{
    const s=p.schedule;
    return <details key={p.id} className="shop-hours-item"><summary><strong>{p.name??p.categories[0]?.replaceAll('_',' ')??'Place listing'}</strong><span>{p.hours.state==='unknown'?'Hours unknown':p.hours.state==='closed'?'Listed closed when you pass':p.hours.basis==='regular'?'Usually open when you pass':'Listed open when you pass'}</span></summary>
      ...
      <h4>Current seven-day schedule</h4>{s?.currentWeek.length?<ul>{s.currentWeek.map((t,i)=><li key={i}>{t}</li>)}</ul>:<p>{analysis.source==='sample'?'Illustrative hours; no Google schedule requested.':'Google did not provide the daily schedule text.'}</p>}
