...
export async function liveComparison(journey: LiveJourney, signal: AbortSignal, accessCode: string): Promise<LiveResult> {
  ...
  try { response = await fetch(`${base}/api/compare`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(accessCode ? { 'X-Nightwise-Code': accessCode } : {}) }, body: JSON.stringify(payload), signal: AbortSignal.any([signal, AbortSignal.timeout(100000)]), cache: 'no-store' }); }
