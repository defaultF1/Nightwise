...
export async function serviceStatus(signal?: AbortSignal): Promise<ServiceStatus> {
  const response = await fetch(`${base}/api/status`, { signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(5000)]) : AbortSignal.timeout(5000), cache: 'no-store' });
...
export async function liveComparison(journey: LiveJourney, signal: AbortSignal, accessCode: string): Promise<LiveResult> {
  ...
  try { response = await fetch(`${base}/api/compare`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(accessCode ? { 'X-Nightwise-Code': accessCode } : {}) }, body: JSON.stringify(journey), signal: AbortSignal.any([signal, AbortSignal.timeout(100000)]), cache: 'no-store' }); }
