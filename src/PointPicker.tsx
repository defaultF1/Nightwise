...
export function PointPicker({ value, onChoose, accessCode='' }: { value: JourneyPoint; onChoose: (point: JourneyPoint) => void; accessCode?:string }) {
  ...
  return <><p className="sheet-intro">Find a demo location or search within 10 km of AEOS in North Bengaluru. Ordinary streets are supported too.</p>
    <PlaceSearch onChoose={onChoose} accessCode={accessCode}/>
    <Favourites point={value} onChoose={onChoose} accessCode={accessCode}/>
    <form className="coordinate-form" onSubmit={e => { e.preventDefault(); const point = { name: name.trim(), latitude: Number(latitude), longitude: Number(longitude) }; if (!point.name || !latitude.trim() || !longitude.trim() || !inPilotArea(point)) setError('Enter a name and coordinates within 10 km of AEOS in North Bengaluru.'); else onChoose(point); }}>
