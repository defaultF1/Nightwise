...
export function loadRoadAnalyzer(file: string) {
  try {
    ...
    if(data.remark||!Array.isArray(data.elements)||data.elements.length>30000) throw new Error('Invalid road extract');
    let points=0;
    const ways:RoadWay[]=data.elements.flatMap((w:any)=>{
      ...
      if(points>250000)throw new Error('Road extract too detailed');
