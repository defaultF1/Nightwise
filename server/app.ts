...
export async function createServer(config: ServerConfig, fetcher?: typeof fetch) {
  ...
  app.post<{ Body: LiveJourney }>('/api/compare', { schema: { body: { type: 'object', additionalProperties: false, required: ['origin', 'destination', 'mode'], properties: { origin: pointSchema, destination: pointSchema, mode: { type: 'string', enum: ['DRIVE'] } } } } }, async (request, reply) => {
    ...
    if (!config.liveEnabled) throw new ServiceError('live-paused', 'Live Google requests are paused while billing is pending. Tutorial mode is ready.');
    const journey = request.body;
    if (!inBengaluru(journey.origin) || !inBengaluru(journey.destination) || distanceMeters(journey.origin, journey.destination) < 100 || distanceMeters(journey.origin, journey.destination) > 15000) throw new ServiceError('outside-area', 'Choose different Bengaluru pins between 100 m and 15 km apart for this pilot.', 422);
