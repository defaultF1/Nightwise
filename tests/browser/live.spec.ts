...
test('configuration checks do not compare; missing server configuration is explicit',async({page})=>{
 ...
 await page.getByRole('button',{name:'Live routes',exact:true}).click();await page.getByRole('button',{name:'Compare night routes'}).click();await expect(page.getByRole('heading',{name:'We couldn’t load routes'})).toBeVisible();await expect(page.getByRole('main')).toContainText('server key');expect(compares).toBe(1);
...
test('live routes have no invented activity and selected corridor handoff survives map failure',async({page})=>{
 ...
 await page.getByRole('button',{name:'Live routes',exact:true}).click();await page.getByRole('button',{name:'Compare night routes'}).click();await expect(page.getByRole('radio',{name:'Fastest',exact:true})).toBeChecked();
