...
function response(){
 ...
 const analyses=result.analyses.map(a=>({...a,source:'live',activityCoverage:.21,coreComparable:false,distanceMeters:1000,segments:[{fromMeters:0,toMeters:210,state:'active'},{fromMeters:210,toMeters:1000,state:'unknown'}],places:[{...a.places[0],name:'Example pharmacy',hours:{state:'closed',closingSoon:false,minutesUntilClose:null,basis:'current'},schedule:{currentWeek:['Wednesday: Closed'],regularWeek:['Monday: 9:00 AM – 10:00 PM','Wednesday: 10:00 AM – 9:00 PM'],specialDates:['2026-09-09'],nextOpenTime:'2026-09-10T03:30:00Z'}}]}));
...
test('explains incomplete coverage and lets users learn the three patterns',async({page})=>{
 const strip=page.locator('.activity-strip').first();await expect(strip).toContainText('Enough information for 21% of this route; 79% is still unknown.');await expect(strip).toContainText('not a safety rating');await strip.getByText('How does this help me choose?').click();await expect(strip).toContainText('This does not mean the road is empty.');await expect(strip.getByText('Destination',{exact:true})).toBeVisible();
