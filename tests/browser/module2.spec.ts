...
test('AEOS to Manyata keeps supplied pins and synchronized sample choices',async({page})=>{
  ...
  await page.getByRole('button',{name:/Select .*safest route on diagram/i}).focus(); await page.keyboard.press('Enter');
  await expect(page.getByRole('radio',{name:/safest route/i})).toBeChecked();
  ...
  await page.getByRole('button',{name:'Keep comparing'}).click(); await expect(page.getByRole('radio',{name:/safest route/i})).toBeChecked();
});
...
test('zero one and three alternatives are truthful',async({page})=>{
 for(const [value,count] of [['none',0],['one',1],['three',3]] as const){
  await page.getByRole('button',{name:'Open settings'}).click();await page.getByText('Tutorial scenarios',{exact:true}).click();await page.getByRole('button',{name:/^Preview scenario:/}).click();await page.getByRole('option',{name:({"none":"No routes returned","one":"One available route","three":"Three route options","error":"Connection problem","normal":"Two route options","limited":"Incomplete activity data","unknown":"All evidence unavailable","capped":"Search result limit","closing":"Places closing soon","detour":"Substantial detour","similar":"Similar activity"} as Record<string,string>)[value],exact:true}).click();await page.getByRole('button',{name:'Done',exact:true}).click();await page.getByRole('button',{name:'Compare night routes'}).click();
...
test('cancellation prevents stale results and sample errors recover',async({page})=>{
 ...
 await page.getByRole('button',{name:'Open settings'}).click();await page.getByText('Tutorial scenarios',{exact:true}).click();await page.getByRole('button',{name:/^Preview scenario:/}).click();await page.getByRole('option',{name:({"none":"No routes returned","one":"One available route","three":"Three route options","error":"Connection problem","normal":"Two route options","limited":"Incomplete activity data","unknown":"All evidence unavailable","capped":"Search result limit","closing":"Places closing soon","detour":"Substantial detour","similar":"Similar activity"} as Record<string,string>)['error'],exact:true}).click();await page.getByRole('button',{name:'Done',exact:true}).click();await page.getByRole('button',{name:'Compare night routes'}).click();
