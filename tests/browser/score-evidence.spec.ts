...
test('a mocked partial score exposes missing road signals without filling them with zero',async({page})=>{
  ...
  await expect(page.locator('.route-card.selected .activity-score')).toContainText('4/6 signals · Partial');
  ...
  await expect(breakdown).toContainText('4 of 6 signals used');
  await expect(breakdown.getByText('Not included',{exact:true})).toHaveCount(2);
  await expect(breakdown).toContainText('missing evidence is never scored as zero');
