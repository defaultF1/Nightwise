...
test('swaps endpoints and confirms both pins before sending a request',async({page})=>{
 ...
 await page.getByRole('button',{name:'Live routes',exact:true}).click();await page.getByRole('button',{name:'Swap origin and destination'}).click();
...
test('saved manual pins survive relaunch and can be removed',async({page})=>{
 await page.getByRole('button',{name:'Live routes',exact:true}).click();await page.locator('.place-field').first().click();await page.getByLabel('Favourite label').fill('Office');await page.getByRole('button',{name:'Save selected pin'}).click();
 await page.reload();await page.getByRole('button',{name:'Live routes',exact:true}).click();await page.locator('.place-field.destination').click();await page.getByRole('button',{name:'Office',exact:true}).click();await expect(page.locator('.place-field.destination')).toContainText('Office');
...
test('favourites keep IDs and refresh details without retaining provider coordinates',async({page})=>{
 ...
 await page.getByRole('button',{name:'Live routes',exact:true}).click();await page.locator('.place-field.destination').click();await page.getByLabel('Search places').fill('Bengaluru Palace');await page.getByRole('button',{name:'Search Bengaluru'}).click();await page.getByRole('button',{name:/Bengaluru Palace Bengaluru/}).click();
 ...
 const saved=await page.evaluate(()=>localStorage.getItem('nightwise.favourites.geoapify.v1'));expect(saved).toContain('palace-id');expect(saved).not.toContain('77.5891');expect(saved).not.toContain('Palace Road');
 ...
 await page.getByRole('button',{name:'Live routes',exact:true}).click();await page.locator('.place-field.destination').click();await page.getByRole('button',{name:'Palace visit',exact:true}).click();expect(calls).toBe(1);
...
test('time preference persists and evidence and freshness are visible',async({page})=>{
 ...
 await page.getByRole('button',{name:'Compare night routes'}).click();await expect(page.getByRole('radio',{name:'Fastest route',exact:true})).toBeChecked();
