...
describe('persistent request allowance', () => {
  ...
  it('allows the hosted service without a team access code', () => { expect(readConfig({HOST:'0.0.0.0'}).host).toBe('0.0.0.0'); expect(readConfig({HOST:'0.0.0.0',PILOT_ACCESS_CODE:'stale-value'}).host).toBe('0.0.0.0'); });
});
...
describe('live backend', () => {
  ...
  it('refuses missing credentials, invalid inputs and disallowed origins before provider calls', async () => {
    ...
    const hosted=await createServer(config({PILOT_ACCESS_CODE:'stale-value'}),calls); try { expect((await hosted.inject('/api/status')).json().accessCodeRequired).toBe(false); expect(calls).not.toHaveBeenCalled(); } finally { await hosted.close(); }
  });
