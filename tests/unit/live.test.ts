...
describe('Google response boundary', () => {
  ...
  it('never converts openNow without a known close horizon into a forecast', () => { const now='2026-09-09T15:00:00Z'; const observation=parseScan({places:[{id:'p',location:path[0],types:['pharmacy'],businessStatus:'OPERATIONAL',currentOpeningHours:{openNow:true}}]},'q',now).scan.places[0]; expect(evaluateObservation(observation,now).state).toBe('unknown'); observation.currentHours!.nextCloseTime='2026-09-09T15:10:00Z'; expect(evaluateObservation(observation,now,20)).toMatchObject({state:'open',closingSoon:true}); expect(evaluateObservation(observation,'2026-09-09T16:00:00Z').state).toBe('unknown'); });
});
describe('live backend', () => {
  ...
  it('skips all nearby calls when full coverage cannot fit remaining budget', async () => { const calls=vi.fn(async()=>new Response(JSON.stringify(response))); const app=await createServer(config({ENABLE_ACTIVITY_ANALYSIS:'true',PILOT_NEARBY_LIMIT:'1'}),calls); try { const res=await app.inject(request); expect(res.json().activityStatus).toBe('budget'); expect(res.json().usage.nearbyCalls).toBe(0); expect(calls).toHaveBeenCalledTimes(1); } finally { await app.close(); } });
