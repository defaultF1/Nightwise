...
it('public saved place resolution still consumes the existing details allowance',async()=>{
 const calls=vi.fn(async()=>new Response(JSON.stringify({id:'place_1',location:path[0],formattedAddress:'Updated address'})));
 const app=await createServer(config({ENABLE_LIVE_REQUESTS:'true',ENABLE_PLACE_SEARCH:'true',PILOT_ACCESS_CODE:'test-private-access',PILOT_DETAILS_LIMIT:'1'}),calls);
 const request={method:'POST' as const,url:'/api/places/saved',payload:{placeId:'place_1'}};
 try{
  expect((await app.inject('/api/status')).json().accessCodeRequired).toBe(false);
  expect((await app.inject(request)).json()).toMatchObject({coordinate:path[0],address:'Updated address'});
  expect((await app.inject(request)).statusCode).toBe(429);expect(calls).toHaveBeenCalledTimes(1);
...
describe('search and pause protection',()=>{
 ...
 it('validates public search input before any provider call',async()=>{const calls=vi.fn();const app=await createServer(config({ENABLE_LIVE_REQUESTS:'true',ENABLE_PLACE_SEARCH:'true',PILOT_ACCESS_CODE:'test-team-code'}),calls);try{expect((await app.inject('/api/status')).json().accessCodeRequired).toBe(false);expect((await app.inject({...suggestion,payload:{query:'x',sessionToken:token}})).statusCode).toBe(400);expect(calls).not.toHaveBeenCalled();}finally{await app.close();}});
