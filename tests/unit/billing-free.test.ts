...
it('saved place resolution needs no access code and consumes the existing details allowance',async()=>{
...
describe('search and pause protection',()=>{
 ...
 it('checks malformed search input before any provider call',async()=>{const calls=vi.fn();const app=await createServer(config({ENABLE_LIVE_REQUESTS:'true',ENABLE_PLACE_SEARCH:'true',PILOT_ACCESS_CODE:'stale-value'}),calls);try{expect((await app.inject({...suggestion,payload:{query:'x',sessionToken:token}})).statusCode).toBe(400);expect(calls).not.toHaveBeenCalled();}finally{await app.close();}});
