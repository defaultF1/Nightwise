...
describe('bounded scan coverage',()=>{
 ...
 it('uses four refinements only within the cap and does not count a failed child as complete',async()=>{const q={id:'q',coordinate:AEOS_PIN,radiusMeters:150};const plan={queries:[q],samplesByRoute:{},totalSamples:1};const nearby=vi.fn(async(query:typeof q)=>({scan:{queryId:query.id,observedAt:TUTORIAL_CHECKED_AT,status:query.id==='q'?'capped':query.id.endsWith(':1:1')?'failed':'ok',places:[]} as NearbyScan,attributions:[]}));const result=await collectScans(plan,{nearby},new AbortController().signal,5);expect(result.calls).toBe(5);expect(result.scans[0].status).toBe('capped');nearby.mockClear();expect((await collectScans(plan,{nearby},new AbortController().signal,4)).calls).toBe(1);});
