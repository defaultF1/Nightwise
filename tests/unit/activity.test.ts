...
function evidence(r:Route,overrides:Partial<ActivityAnalysis>={}):ActivityAnalysis {const plan=buildQueryPlan([r]);return {...analyzeRoute(r,plan,plan.queries.map(q=>({queryId:q.id,observedAt:now,status:'ok',places:[]})),now),openPlaces:4,potentialHelpPoints:1,longestLowActivityMeters:200,...overrides};}
describe('comparison invariants',()=>{
 it('uses all six weighted signals when comparable inputs exist',()=>{
  ...
  const result=compareActivity([a,b],[evidence(a,{openTransportPoints:0}),evidence(b,{openPlaces:8,potentialHelpPoints:2,longestLowActivityMeters:0,openTransportPoints:3})],{a:{mainRoadFraction:.5,maneuversPerKm:5},b:{mainRoadFraction:1,maneuversPerKm:0}});
 ...
 it('normalizes counts by distance and does not reward a longer equal-density route',()=>{const a=route(1000,'a'),b=route(2000,'b');const ea=evidence(a),eb=evidence(b,{openPlaces:8,potentialHelpPoints:2});expect(componentValues(ea).openDensity).toBe(componentValues(eb).openDensity);expect(componentValues(ea).helpDensity).toBe(componentValues(eb).helpDensity);expect(compareActivity([a,b],[ea,eb]).outcome).toBe('similar');});
 ...
 it('does not force a large detour despite stronger activity',()=>{const a=route(1000,'a'),b={...route(1000,'b'),durationSeconds:1800};const result=compareActivity([a,b],[evidence(a,{openPlaces:0,potentialHelpPoints:0,longestLowActivityMeters:1000}),evidence(b,{openPlaces:8,potentialHelpPoints:2,longestLowActivityMeters:0})]);expect(result.outcome).toBe('detour');expect(result.recommendedId).toBeNull();expect(result.selectedId).toBe('a');});
