...
Provider opening schedules are evaluated at estimated passing times, using step-duration distribution when available. The Night Activity Score v7 adds fixed contributions without expanding available factors to fill 100. Car/bike allocations are open places 25, main roads 15, help 15, continuity 15, simplicity 10, transport 10 and mapped cameras 10. Walking uses 25, 0, 25, 25, 5, 10 and 10 respectively. Camera points combine eligible mapped-site density and distribution; private/indoor records and colocated duplicates are excluded. Cameras no longer receive a second ranking tie-break. Density grows gradually and typical-hours estimates receive 35% opening credit. See [the formula and assumptions](docs/night-activity-score-v7.md). The evidence sheet shows category counts from observations and only evaluated score factors. Automatic recommendations require sufficient shared factors, a ten-point advantage and the extra-time preference; estimated comparisons keep the fastest selection. These are listing estimates, not statistical confidence or safety predictions. Local calibration remains an acceptance dependency. Community reporting is outside this version; anonymous route feedback (a thumbs rating with route label and city only) is stored via `/api/feedback`. Bengaluru Palace is an example search, not an invented entrance preset.
