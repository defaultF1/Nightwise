...
Deploy the `codex/journey-updates` branch as a Node web service with build command `npm ci --include=dev && npm run build:server` and start command `node build-server/index.mjs`. Set `HOST=0.0.0.0` and `PORT=10000`. Use `/api/status` as the health path. Keep the Google request switches off during setup. The team preview has no login or access-code prompt; its shared Google request limits remain the protection against overspend.
