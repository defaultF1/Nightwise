...
Google opening schedules are evaluated at estimated passing times, using step-duration distribution when available. The Night Activity Score combines up to six observed signals with fixed weights (open places 25, main roads 20, help 15, continuity 15, simplicity 15, transport 10), rescaled to 100 across the signals available for every route; missing signals are left out, never scored as zero. The evidence sheet shows plain category counts (medical stores, hospitals, petrol pumps, shops) from what was observed. Automatic recommendations require a ten-point advantage within the extra-time preference. These are listing estimates, not statistical confidence or safety predictions. Local calibration remains an acceptance dependency. Community reporting is outside this version; anonymous route feedback (a thumbs rating with route label and city only) is stored via `/api/feedback`. Bengaluru Palace is an example search, not an invented entrance preset.
